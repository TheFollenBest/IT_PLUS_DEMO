                    ...
                    <!-- Заголовок -->
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="editor_html" name="code_html" class="form-control" rows="4">{{ $lesson['code_html'] }}</textarea>

                                <label for="editor_html_answer">Код HTML <span style="color:grey">(ответ)</span></label>
                                <textarea id="editor_html_answer" name="answer_code_html" class="form-control" rows="4">@foreach($answers->where('id_lesson', $lesson['id']) as $answer){{ $answer->answer_code_html }}@endforeach</textarea>
                            </div>
                            <!-- Поле код CSS -->
                            <div class="form-group">
                                <label for="editor">Код CSS <span style="color:grey">(необязательно)</span></label>
                                <textarea id="editor_css" name="code_css" class="form-control" rows="4">{{ $lesson['code_css'] }}</textarea>

                                <label for="editor_css_answer">Код CSS <span style="color:grey">(ответ)</span></label>
                                <textarea id="editor_css_answer" name="answer_code_css" class="form-control" rows="4">@foreach($answers->where('id_lesson', $lesson['id']) as $answer){{ $answer->answer_code_css }}@endforeach</textarea>
                            </div>
                            <!-- Поле код JS -->
                            <div class="form-group">
                                <label for="editor">Код JS <span style="color:grey">(необязательно)</span></label>
                                <textarea id="editor_js" name="code_js" class="form-control" rows="4">{{ $lesson['code_js'] }}</textarea>

                                <label for="editor_js_answer">Код JS <span style="color:grey">(ответ)</span></label>
                                <textarea id="editor_js_answer" name="answer_code_js" class="form-control" rows="4">@foreach($answers->where('id_lesson', $lesson['id']) as $answer){{ $answer->answer_code_js }}@endforeach</textarea>
                            </div>
                            <!-- Поле другой код -->
                            <div class="form-group">
                                <label for="editor">Другой код <span style="color:grey">(необязательно)</span></label>
                                <textarea id="editor_other" name="code_other" class="form-control" rows="4">{{ $lesson['code_other'] }}</textarea>

                                <label for="editor_other_answer">Другой код <span style="color:grey">(ответ)</span></label>
                                <textarea id="editor_other_answer" name="answer_code_other" class="form-control" rows="4">@foreach($answers->where('id_lesson', $lesson['id']) as $answer){{ $answer->answer_code_other }}@endforeach</textarea>
                            </div>
                            <!-- Поле методичка -->
                            <div class="form-group">
